...
cursor.execute(f"""
    CREATE TABLE IF NOT EXISTS {nome_tabela} (
        ...
        status VARCHAR(255) DEFAULT 'Drive' NOT NULL
    );
    """)
...
SERVICE_ACCOUNT_FILE = 'token.json' #token para acesso drive
SHARED_DRIVE_ID = #ID da pasta no drive para armazenar as NFs
    ...
    try:
        cursor.execute(f"""
            INSERT INTO {nome_tabela} (
                ...
                '{n_conta}', '{nome_sql}', '{valor_total}', '{juros}', '{vencimento}', '{data}', '{data_emissao}', DEFAULT
            )
        """)
        db.commit()
    except:
...
with open("claro.txt", 'r') as arquivo:
    for linha in arquivo:
        ...
        if login == #numero de login para acesso as contas:
            verificando_conta('')#numero da conta para verificar disponibilidade da fatura
            sleep(2)
            verificando_data()
